...
### Social sharing images — DONE

All 18 pages now carry `og:image` + `og:image:alt`:

- **12 product pages** point at the product photograph they already display,
  so no new asset was needed.
- **6 other pages** (home, both brand listings, about, contact, 404) share
  `assets/img/og/og-default.jpg` — 1200×630, generated by
  `tools/make-og-image.ps1` from the site's own palette, knit texture and
  brand names. Re-run that script if the brand names or colours change.

URLs are **absolute** (`https://panwarknitwear.com/...`), matching the
existing `og:url` and `canonical` tags — crawlers ignore relative paths.

`tools/verify.ps1` now asserts this on every page: og:image present wherever
`twitter:card` is declared, absolute, resolving to a file on disk, and paired
with `og:image:alt`.

> **If the site moves to a different domain**, `og:image`, `og:url` and
> `canonical` all need updating together. They currently all point at
> `panwarknitwear.com`.
